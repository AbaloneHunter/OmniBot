...
class _AgentConfigPageState extends State<AgentConfigPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (agent.builtIn) {
        payload = await CodexAppServerService.readAgentConfig(agent.id);
      }
      if (!mounted) return;
      _syncAgent(agent);
    ...
    try {
      switch (_kind) {
        case 'codex':
          ...
          final payload = await CodexAppServerService.writeAgentConfig(
            _agent!.id,
            baseUrl: baseUrl,
            model: model,
            apiKey: apiKey,
        ...
        case 'profile':
          ...
          final catalog = await CodexAppServerService.saveAgent(
            AcpAgentProfile(
              id: _agent!.id,
              name: _agent!.name,
  ...
  Future<void> _deleteCustomAgent() async {
    ...
    try {
      await CodexAppServerService.deleteAgent(agent.id);
      if (!mounted) return;
